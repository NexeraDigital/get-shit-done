...
must_haves:
  ...
  artifacts:
    ...
    - path: "autopilot/dashboard/src/components/PhaseCard.tsx"
      provides: "Current phase card (DASH-11)"
      min_lines: 15
...
- If no phases, show "No phases loaded" placeholder
- Tailwind: card with border rounded-lg shadow-sm p-6, status badge with rounded-full px-2 text-xs
...
Run `cd autopilot/dashboard && npx vite build` -- succeeds.
  ...
  <done>Overview page renders all 5 dashboard components (DASH-10 through DASH-14) with responsive grid layout, store subscriptions, auto-scrolling logs, and question call-to-action linking to the question response page.</done>
</task>
...
<success_criteria>
Navigating to the root URL shows a complete overview dashboard with progress bar, current phase card, question badge, activity feed, and collapsible log stream -- all wired to the Zustand store for real-time updates via SSE.
</success_criteria>
