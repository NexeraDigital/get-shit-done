...
must_haves:
  truths:
    ...
    - "Orchestrator stops starting new phases when shutdown is requested"
...
1. Check `this.shutdownRequested` -- if true, return
...
5. Call `await fn()`
6. Update `phase.steps[stepName]` to 'done', persist state
7. Emit 'step:completed' event
8. Log via `logger.log('info', 'orchestrator', `Step ${stepName} completed for phase ${phase.number}`)`
...
- Otherwise: call `executeWithRetry('/gsd:discuss-phase ${phase.number}', ...)` (DISC-01 through DISC-03 delegated to GSD command)
...
4. Log retry: `logger.log('warn', 'orchestrator', 'Command failed, retrying once', ...)`
5. Second attempt: same call
6. If success: return result
7. Escalate: emit 'error:escalation' event with error details and retry/skip/abort options (ORCH-04)
8. For Phase 3 (no web UI yet): log the escalation to console and default to 'abort' behavior (throw error). The web UI response mechanism is Phase 4. For now, the orchestrator emits the event and throws -- callers can listen and decide.
9. Record error in state.errorHistory (ORCH-06)
...
11. **Orchestrator stops on shutdown request:**
...
Run `cd autopilot && npm test` -- full suite must pass (no regressions).
  ...
  <done>
Orchestrator is fully tested with 14 test cases covering: step sequencing, resume, state persistence ordering, retry/escalate, skip-discuss, skip-verify, gap detection, gap cap, shutdown, events, YOLO config, and phase range filtering. All tests pass alongside the full suite.
  </done>
</task>
...
- Shutdown flag prevents new phases/steps from starting
- All 14 tests pass
